// One-call presentation of the native CrashReportView. The embeddable view
// lives in CrashReportView.swift; this pops it as a sheet (iOS) or window
// (macOS). The web flow (presentURL / hosted page) remains an option.
...
extension RetentionFlow {
    /// Present the **native** crash-report form as a sheet — the native
    /// alternative to opening the hosted web page. Renders the published
    /// config (message + optional email) with device/OS/app diagnostics
    /// attached, pre-filling a crash captured by
    /// ``enableCrashDetection()`` when one is pending.
    ///
    /// - Parameters:
    ///   - userId: Stable user id (optional; reports are anonymous by default).
    ///   - flowSlug: Target a non-primary crash flow. Omit for the default.
...
extension RetentionFlow {
    nonisolated(unsafe) private static var crashWindowController: NSWindowController?

    /// Present the **native** crash-report form in a window (macOS).
    ...
    public static func presentCrashReport(
        ...
        crashWindowController = controller
